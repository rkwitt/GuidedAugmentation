...
a selection of desired depths/poses. Using this architecture, for a new image, we are able to 
...
first requires a a-priori trained attribute regressor $\gamma$ for each given 
...
of the object attibute range and a desired object attribute value $t_k$, we partition
...
One $\phi_i^k$ is then learned from $\mathcal{S}_i$ for each desired
object attribute value $t_k$. As 
training is done in feature space $\mathcal{X}$, we 
...
if we would \emph{only} use the ground-truth bounding to
...
is [0.23m, 0.75m] for \texttt{Depth} and [0$^\circ$, 180$^\circ$] for \texttt{Pose}.}
...
This setup results in $T=11$ target values for 
...
however, the $L_2$ norm is hard to interpret. Instead, we suggest to compute the 
...
following one-shot experiments of Sec.~\ref{subsection:one-shot}.
