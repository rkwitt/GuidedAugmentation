...
of transfer based methods, however, has not yet been tested under more severe constrains such as 
...
depth and pose, will hallucinate\mnl{Is hallucinate standard terminology here?} the changes in its RCNN features for 
...
Sec.~\ref{section:architecture} introduces
...
demonstrates that AGA in feature space considerably improves 
...
\noindent
\textbf{Objective.}
On a conceptual level, we construct a mapping function $\phi$ which,
...
$\phi_i^k$ for a selection of intervals $[l_i,h_i]$\mnl{Should this be $t \in [l_i,h_i]$? What exactly are these intervals of?}
and a selection of $K$ given target attribute values $t_k$\mnl{I am not following this. Are you saying that each interval has a fixed, constant target attribute value?}. While
this simplifies the problem, it requires \mn{a good a-priori \emph{attribute 
predictor}}, since, otherwise, we could not decide which 
$\phi_i$ to use\mnl{What is the difference between $\phi_i$ and $\phi_i^k$?}. During testing, we (1) predict the attribute strength, \ie, 
$\gamma(\mathbf{x}) =
\hat{t}$, and then (2) synthesize features as
$\hat{\mathbf{x}} = 
\phi_i^k(\mathbf{x})$ for $k=1,\ldots,T$ if $\hat{t} 
\in [l_i,h_i]$\mnl{I do not understand how the predicted value $\hat{t}$ is used in feature synthesis. Is the goal not to synethesize features for all attributes?}. Next, we discuss each component of this architecture in detail.
...
\caption{Architecture of the attribute regressor $\gamma$.\mnl{I do not understand the symbols in this graph.} \label{fig:COR}}
\end{figure}
...
of the attribute range.\mnl{I am not following why binning is needed at all.}
...
target attribute value $t_k$\mnl{Is this a fixed value in the middle of the interval? When using the network for feature synthesis do you still need the feature predictor or do you simply run the input through all $K$ networks? I am a bit confused here.}, we partition
...
with the 2D ground-truth bounding boxes\mnl{Not totally clear to me. Why are you using all of thes proposals? Just to crate more input data, even though some of these proposals do not exactly capture the objects of interest?}.
...
\texttt{Pose}.\mnl{Should there be an average over all these results? Also, what is the typical range for these values? I.e., is a depth error of 0.5 m good or not? Lastly, pose errors seem to be enormous. And what does an error >90 degrees mean for object that are approximately symmetric?}}
\end{table}
...
\texttt{Depth} and 7 for \texttt{Pose}. \mnl{I really cannot follow what the $\Phi_i^k$ are for.}
...
target values\mnl{Is there not exactly one target value for each $\Phi_i^k$?}. To accomplish this, we run each synthetic feature $\hat{\mathbf{x}}$ through
...
differ from original features. \mnl{I do not follow this? Can you not compute the true features for the test data and compare these features to the synthesized ones? What am I missing?}While we cannot guarantee an identity
...
collections\mnl{Why do you randomly select two collections?} of 10 object classes to assess the quality
of AGA on different sets of unseen objects. We ensure
that each object class has at least 100 samples in the 
testing split of SUN RGB-D and that no object class \mn{is}\mnl{correct?}
...
\vskip1ex
As a \emph{Baseline} comparison\mnl{Is this a standard thing to do? Do I understand this correctly that this means that for each class you only have one training instance?}, we train a linear C-SVM
...
features synthesized by our approach\mnl{Grammar? What do you mean?}. We repeat the 
...
this effect to the difficuly in predicting object pose from 2D data, as can
...
over three different sets $\mathcal{T}_i$ of (unseen) object classes.\mnl{What is the point of this figure if you have Table~\ref{table:oneshot}> Does it not show the same information?}}
\end{figure}
