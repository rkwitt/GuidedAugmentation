function DataMatrix = SUNRGBD_consolidate( MetaData_withFeatures, config )
...
for i=1:Nimages
    ...
    Ndetections = Ndetections + ...
        size( MetaData_withFeatures(i).CNN_features, 1 );
...
for i=1:Nimages
   ...
   % e.g., image 1, 5 features => 1 1 5
   %       image 2, 2 features => 2 6 2
   %       ...
...
DataMatrix = single( DataMatrix );
save( fullfile( config.outdir, 'DataMatrix.mat' ), 'DataMatrix', '-v7.3' );
save( fullfile( config.outdir, 'DataMatrix_img2idx.mat' ), 'DataMatrix_img2idx', '-v7.3');
